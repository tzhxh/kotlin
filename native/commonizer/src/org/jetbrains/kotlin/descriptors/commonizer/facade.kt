...
private fun mergeAndCommonize(storageManager: StorageManager, parameters: CommonizerParameters): CirTreeMergeResult {
    ...
    val classifiers = CirKnownClassifiers(
        ...
        dependencies = mapOf(
            ...
            parameters.sharedTarget to CirProvidedClassifiersImpl.create(parameters.dependencyModulesProvider)
        )
    )
...
private fun diffTrees(oldRootNode: CirRootNode, newRootNode: CirRootNode) {
    ...
    fun <T : Any> checkKeys(where: String, oldKeys: Set<T>, newKeys: Set<T>): Collection<Pair<T, T>> {
        ...
        if (justOldKeysStrToKey.isNotEmpty())
            println("JUST_OLD(${justOldKeysStrToKey.size}) $where: " + justOldKeysStrToKey.keys.sorted())

        if (justNewKeysStrToKey.isNotEmpty())
            println("JUST_NEW(${justNewKeysStrToKey.size}) $where: " + justNewKeysStrToKey.keys.sorted())

        if (justOldKeysStrToKey.size != justNewKeysStrToKey.size)
            print("")
    ...
    fun checkType(where: String, what: String, old: CirType?, new: CirType?) {
        ...
        if (old == null || new == null) {
            println("$where > $what: missing old ${old == null}, new ${new == null}")
        } else if (old == new || new == CirTypeFactory.StandardTypes.ANY) {
            return
        } else {
            println("$where > $what: $old != $new")
        }
    ...
    fun checkTypes(where: String, what: String, olds: List<CirType>, news: List<CirType>) {
        if (olds.size != news.size) {
            println("$where > $what.size: ${olds.size} != ${news.size}")
        } else {
    ...
    fun checkAnnotation(where: String, old: CirAnnotation, new: CirAnnotation) {
        ...
        if (old.constantValueArguments != new.constantValueArguments)
            println("$where > constantValueArguments: ${old.constantValueArguments} != ${new.constantValueArguments}")

        if (old.annotationValueArguments.keys != new.annotationValueArguments.keys)
            println("$where > annotationValueArguments.keys: ${old.annotationValueArguments.keys} != ${new.annotationValueArguments.keys}")
    ...
    fun checkAnnotations(where: String, what: String, olds: List<CirAnnotation>?, news: List<CirAnnotation>?) {
        if (olds == null && news == null)
            return

        if (olds == null || news == null) {
            println("$where > $what: missing old ${olds == null}, new ${news == null}")
        } else if (olds.size != news.size) {
            println("$where > $what.size: ${olds.size} != ${news.size}")
        } else {
            for (i in olds.indices) {
                val old = olds[i]
    ...
    fun checkModifiers(where: String, old: CirFunctionModifiers, new: CirFunctionModifiers) {
        if (old != new)
            println("$where > modifiers: $old != $new")
    }
    ...
    fun checkContainingClass(where: String, old: CirContainingClass?, new: CirContainingClass?) {
        ...
        if (old == null || new == null) {
            println("$where > containingClass: missing old ${old == null}, new ${new == null}")
        } else {
    ...
    fun checkReceiver(where: String, old: CirExtensionReceiver?, new: CirExtensionReceiver?) {
        ...
        if (old == null || new == null) {
            println("$where > receiver: missing old ${old == null}, new ${new == null}")
        } else {
            checkAnnotations("$where > receiver", "annotations", old.annotations, new.annotations)
    ...
    fun checkValueParameters(where: String, olds: List<CirValueParameter>, news: List<CirValueParameter>) {
        if (olds.size != news.size) {
            println("$where > valueParameters.size: ${olds.size} != ${news.size}")
        } else {
            for (i in olds.indices) {
                val old = olds[i]
                val new = news[i]
    ...
    fun checkFunctions(where: String, old: CirFunction?, new: CirFunction?) {
        ...
        if (old == null || new == null) {
            println("$where: missing old ${old == null}, new ${new == null}")
        } else {
    ...
    fun checkConstant(where: String, old: CirConstantValue<*>?, new: CirConstantValue<*>?) {
        ...
        if (old == null || new == null) {
            println("$where > constant: missing old ${old == null}, new ${new == null}")
        } else if (old != new) {
            println("$where > constant: $old != $new")
        }
    }
    ...
    fun checkGetter(where: String, old: CirPropertyGetter?, new: CirPropertyGetter?) {
        ...
        if (old == null || new == null) {
            println("$where > getter: missing old ${old == null}, new ${new == null}")
        } else {
    ...
    fun checkSetter(where: String, old: CirPropertySetter?, new: CirPropertySetter?) {
        ...
        if (old == null || new == null) {
            println("$where > setter: missing old ${old == null}, new ${new == null}")
        } else {
    ...
    fun checkProperties(where: String, old: CirProperty?, new: CirProperty?) {
        ...
        if (old == null || new == null) {
            ...
            checkAnnotations(where, "backingFieldAnnotations", old.backingFieldAnnotations, new.backingFieldAnnotations)
            checkAnnotations(where, "delegateFieldAnnotations", old.delegateFieldAnnotations, new.delegateFieldAnnotations)
    ...
    checkKeys(
        ...
        checkKeys(
            ...
            checkKeys(
                "$shortModuleName > $packageName > TAs",
                ...
                for (i in 0 until oldTypeAliasNode.targetDeclarations.size) {
                    checkTypeAliases(
                        "$shortModuleName > $packageName > TAs > $typeAliasName[$i]",
