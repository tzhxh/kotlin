...
internal class LambdaMetafactoryArgumentsBuilder(
    ...
    private fun checkMethodSignatureCompliance(
        ...
        for ((implParameter, methodParameter) in implParameters.zip(methodParameters)) {
            ...
            if (!checkTypeCompliesToConstraint(implParameter.type, constraint))
                return false
        }
        if (!checkTypeCompliesToConstraint(implFun.returnType, constraints.returnType))
            return false
        return true
    }

    private fun checkTypeCompliesToConstraint(irType: IrType, constraint: TypeAdaptationConstraint?): Boolean =
        when (constraint) {
            null -> true
